...
export class SpatialHash {
  ...
  nearest(origin: THREE.Vector3, range: number, entities: readonly Entity[]): Entity | undefined {
    ...
    for (let ox = -cells; ox <= cells; ox++) {
      for (let oz = -cells; oz <= cells; oz++) {
        ...
        for (const index of bucket) {
          ...
          if (entity.dead) continue
          const d2 = entity.position.distanceToSquared(origin)
          if (d2 < bestD2) {
            bestD2 = d2
            best = entity
          }
  ...
  forEachNearby(origin: THREE.Vector3, radius: number, entities: readonly Entity[], visit: (entity: Entity, distanceSquared: number) => void): void {
    const cx = this.cell(origin.x)
    const cz = this.cell(origin.z)
