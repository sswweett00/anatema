...
export default function LootRenderer() {
  ...
  const material = useMemo(
    () => new THREE.MeshBasicMaterial({ toneMapped: false, transparent: true, opacity: 0.95, vertexColors: true }),
    [],
  )
  ...
  useFrame((state) => {
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const hex = finite(item.colorHex ?? 0xffc56e, 0xffc56e)
        if (mesh.instanceColor == null) mesh.setColorAt(i, new THREE.Color(hex))
        else mesh.setColorAt(i, new THREE.Color(hex))
      }
      ...
      console.error('[ANATHEMA] LootRenderer frame failed; disabling loot draw for this frame', error)
      mesh.count = 0
    }
  })
