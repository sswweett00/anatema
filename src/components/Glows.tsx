...
/*
 * IŞIK BİRİKİNTİLERİ — sahneye "bloom" hissi veren, compositor gerektirmeyen
 * additif zemin parıltıları. Saf three.js: her WebGL bağlamında garantili.
 *   · 6 meşale alevinin altında titreyen kor havuzları
 *   · şövalyenin kor kalbinin altında gezinen parıltı
 *   · ayin halkasının merkezindeki zayıf köz
 */
...
export default function Glows() {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < torchRefs.current.length; i++) {
      ...
      const f =
        0.8 +
        Math.sin(t * 11 + seeds[i]) * 0.14 +
        Math.sin(t * 23 + seeds[i] * 2.3) * 0.08
