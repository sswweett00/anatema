...
function CrashScreen({ error, onRetry }: { error: Error; onRetry: () => void }) {
  return (
    ...
    >
      ...
      >
        <div style={{ color: '#ff8a3d', letterSpacing: '.22em', fontSize: 12, fontWeight: 800 }}>
          ANATHEMA
        </div>
        <h1 style={{ margin: '10px 0 8px', fontSize: 32 }}>Oyun başlatılamadı</h1>
        <p style={{ opacity: 0.78, lineHeight: 1.6 }}>
          Grafik veya runtime başlatılırken bir hata oluştu. Oyun siyah ekranda kalmak yerine hata durumunu gösteriyor.
        </p>
        <code
          style={{
            display: 'block',
...
function GlobalRuntimeGuard({ children }: Props) {
  if (typeof window === 'undefined') return <>{children}</>
  return <RuntimeGuardInner>{children}</RuntimeGuardInner>
}
...
function RuntimeGuardInner({ children }: Props) {
  ...
  React.useEffect(() => {
    ...
    const onRejection = (event: PromiseRejectionEvent) => {
      ...
      const error = event.reason instanceof Error ? event.reason : new Error(String(event.reason ?? 'Promise rejection'))
      setFatal(error)
    }
...
export default class BootGuard extends Component<Props, State> {
  ...
  render() {
    return (
      <ErrorBoundary>
        <GlobalRuntimeGuard>{this.props.children}</GlobalRuntimeGuard>
      </ErrorBoundary>
    )
  }
}
